...
uint32_t readPC_LEB_u32(void) {
    ...
    if (!(val & 0x80)) {
        ...
        res = (uint32_t) read_leb(pc, &off, 32, false);
        pc += off;
    }
    return res;
}
...
uint8_t readPC_LEB_u8(void) {
    ...
    if (!(val & 0x80)) {
        ...
        res = (uint8_t) read_leb(pc, &off, 32, false);
        pc += off;
    }
    return res;
}
    ...
    while (1) {
        ...
        switch (opcode) {
            ...
            case Loop:
                ...
                value_type = readPC_LEB_u32();//read_LEB_unsigned(bytes, &m->pc, 32);
                (void) value_type;

                // 如果调用栈溢出，则记录异常信息并返回 false 退出虚拟机执行
                /*放到push的时候检查不好么  by ljp
            ...
            case BrIf:
                ...
                depth = readPC_LEB_u32();//read_LEB_unsigned(bytes, &m->pc, 32);
                // 将操作数栈顶值弹出，作为判断条件
                cond = popStack_u32();//stack[m->sp--].value.uint32;
                // 如果为真则跳转，否则不跳转
                if (cond) {
                    // 将目标控制块关联的栈帧设置为当前栈帧
                    //m->csp -= (int) depth;
            ...
            case BrTable: {
                ...
                uint32_t count = readPC_LEB_u32();//read_LEB_unsigned(bytes, &m->pc, 32);
                ...
                int32_t didx = popStack_i32();//stack[m->sp--].value.int32;
                a = 0 ;
                // 构造索引表
                for (uint32_t n = 0; n < count; n++) {
                    //m->br_table[n] = readPC_LEB_u32();//read_LEB_unsigned(bytes, &m->pc, 32);
                    if (didx == n){
                        depth = readPC_LEB_u32();
                        a = 1;
                    }else {
                        readPC_LEB_u32();
                    }
                ...
                // 读取默认索引
                defaultDepth = readPC_LEB_u32();//read_LEB_unsigned(bytes, &m->pc, 32);
            ...
            case Call:
                ...
                fidx = readPC_LEB_u32();//read_LEB_unsigned(bytes, &m->pc, 32);
            ...
            case Select:
                ...
                uint32_t a = popStack_u32();
                uint32_t b = popStack_u32();
            ...
            case LocalSet:
                ...
                //setLocal(idx, popStack_u32());
                stackTopToLocal(idx, true);
                continue;
            case LocalTee:
                ...
                //stack[m->fp + idx] = stack[m->sp];
                //setLocal(idx, getStack_u32());
                stackTopToLocal(idx, false);
                continue;
            ...
            case I32Load ... I64Load32U:
                ...
                //pushStack_u32(0);//这里不压，我们用临时变量保存，之后获得具体数值后在压栈

                // 根据具体指令将实际内存地址里保存的数值拷贝到操作数栈顶
                switch (opcode) {
                    case I32Load:
                        // 从内存拷贝 4 个字节数到操作数栈顶（栈顶类型为 32 位整数）
                        memcpy(&tmpu32, maddr, 4);
                        //stack[m->sp].value_type = I32;
                        pushStack_u32(tmpu32);
                        break;
                    ...
                    case I64Load:
                        ...
                        //没想好怎么实现呢，等堆栈支持64位再说 by ljp
                        break;
                    case F32Load:
                        // 从内存拷贝 4 个字节数到操作数栈顶（栈顶类型为 32 位浮点数）
                        memcpy(&tmpu32, maddr, 4);
                        //stack[m->sp].value_type = F32;
                        pushStack_u32(tmpu32);//不考虑类型，直接存入4字节就好。如果有类型检查怎么干还没想好 by ljp
                        break;
                    case F64Load:
            ...
            case I32Store ... I64Store32:
                ...
                tmpu32 = popStack_u32();//暂时不考虑64位的情况  by ljp
            ...
            case I32Eq ... I32GeU:
                ...
                a = popStack_u32();//stack[m->sp - 1].value.uint32;
                b = popStack_u32();// stack[m->sp].value.uint32;
                //m->sp -= 1;
                switch (opcode) {
                    case I32Eq:
                        c = a == b;
            ...
            case F32Eq ... F32Ge:
                ...
                g = popStack_f32();//stack[m->sp - 1].value.f32;
                h = popStack_f32();// stack[m->sp].value.f32;
                //m->sp -= 1;
                switch (opcode) {
                    case F32Eq:
                        c = g == h;
            ...
            case I32Clz ... I32PopCnt:
                ...
                a = popStack_u32();//stack[m->sp].value.uint32;
                switch (opcode) {
                    case I32Clz:
                        // 数值的二进制表示的位数
            ...
            case TruncSat: {
                ...
                //readPC_LEB_u32();uint8_t readPC_LEB_u8(void)
                uint8_t type = readPC_LEB_u8();//read_LEB_unsigned(bytes, &m->pc, 8);
                float tmpf32;
                switch (type) {
                    case 0x00:
                        ...
                        OP_I32_TRUNC_SAT_F32(tmpi32, tmpf32)
                        // stack[m->sp].value_type = I32;
                        pushStack_u32(tmpi32);
                        break;
                    case 0x01:
                        // 指令作用：将 32 位浮点数截断为 32 位无符号整数（截掉小数部分）
                        tmpf32 = popStack_f32();
                        // OP_U32_TRUNC_SAT_F32(stack[m->sp].value.uint32, stack[m->sp].value.f32)
                        OP_U32_TRUNC_SAT_F32(tmpu32, tmpf32)
                        //stack[m->sp].value_type = I32;
                        pushStack_u32(tmpu32);
                        break;
                    case 0x02:
